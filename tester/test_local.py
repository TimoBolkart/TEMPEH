...
def execute_locally(test_config_fname):
    from tester.local_tester import run
    run(config_fname=test_config_fname)

def run(model_run_dir, data_list_fname, coarse_prediction_root_dir, image_directory, calibration_directory, image_file_ext, out_dir):
    ...
    if not os.path.exists(data_list_fname):
        ...
        return
    if not os.path.exists(coarse_prediction_root_dir):
        print('Directory of the coarse TEMPEH prediction not found - %s' % coarse_prediction_root_dir)
        return
    if not os.path.exists(image_directory):
        print('Input image directory not found - %s' % image_directory)
        return
    ...
    np.savez(test_config_fname, local_run_dir=model_run_dir, global_registration_root_dir=coarse_prediction_root_dir, 
            out_dir=out_dir, data_list_fname=data_list_fname, image_directory=image_directory, 
...
def main():
    ...
    parser.add_argument('--refinement_model_run_dir', default='./runs/refinement/refinement_TEMPEH_final', help='Path of the TEMPEH refinement model directory')
    parser.add_argument('--data_list_fname', default='./data/test_data_subset/paper_test_frames.json', help='Path of the data list file')
    parser.add_argument('--coarse_prediction_root_dir', default='./results/FaMoS_test_subset/coarse__TEMPEH_final', help='Path of the predicted TEMPEH coarse meshes')
    ...
    parser.add_argument('--out_dir', default='./results/FaMoS_test_subset/refinement__TEMPEH_final', help='Output directory for the predicted meshes')

    args = parser.parse_args()
    model_run_dir=args.refinement_model_run_dir
    data_list_fname=args.data_list_fname
    coarse_prediction_root_dir=args.coarse_prediction_root_dir
    ...
    run(model_run_dir=model_run_dir, data_list_fname=data_list_fname, coarse_prediction_root_dir=coarse_prediction_root_dir, image_directory=image_directory, 
