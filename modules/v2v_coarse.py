...
class V2VModel(nn.Module):
    def __init__(self, input_channels, output_channels, encdec_level=5, decoder_scale_factor=1, norm='bn'):
        super().__init__()
        ...
        self.back_layers = nn.Sequential(
            Res3DBlock(32, 32, norm=norm),
            Basic3DBlock(32, 64, 1, norm=norm),
            Basic3DBlock(64, 128, 1, norm=norm),
        )

        self.output_layer = nn.Conv3d(128, output_channels, kernel_size=1, stride=1, padding=0)
...
def test():
    ...
    model = V2VModel(input_channels=C, output_channels=1000, encdec_level=5, norm='bn').cuda()
